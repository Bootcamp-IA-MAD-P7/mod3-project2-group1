...
export function ConversationStatus({ scenario }: ConversationStatusProps) {
  ...
  const label = t(scenario.labelKey)
    ...
    <section aria-labelledby="conversation-status-title" className={"rounded-2xl border p-5 sm:p-6 " + style.className}>
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="flex items-start gap-3">
          ...
          <div>
            <p className="text-sm font-semibold uppercase tracking-wide">{t("conversation.status")}</p>
            <h2 id="conversation-status-title" className="mt-1 text-xl font-semibold tracking-tight">{label}</h2>
            <p className="mt-2 max-w-2xl text-sm leading-6 opacity-85">{t(scenario.descriptionKey)}</p>
          </div>
        </div>
        <Badge variant="outline" className="border-current/25 bg-white/40 text-inherit dark:bg-slate-950/20">{label}</Badge>
      </div>
      ...
      {scenario.id === "expired" ? (
        <p className="mt-5 flex items-start gap-2 rounded-xl bg-white/45 p-3 text-sm leading-6 dark:bg-slate-950/20">
          ...
          {t("conversation.expiredMessage")}
        </p>
      ) : scenario.job && (scenario.id === "queued" || scenario.id === "processing") ? (
        <div className="mt-5 grid gap-3 text-sm sm:grid-cols-2">
          <StatusDetail label={t("conversation.obtained")} value={String(scenario.job.fetchedCount)} />
          <StatusDetail label={t("conversation.analyzed")} value={String(scenario.job.analyzedCount)} />
        </div>
      ) : null}

      {scenario.job?.errorMessageKey && (
        <p role="alert" className="mt-4 flex items-start gap-2 rounded-xl bg-white/45 p-3 text-sm leading-6 dark:bg-slate-950/20">
          ...
          {t(scenario.job.errorMessageKey)}
        </p>
      )}
      {scenario.job && scenario.id !== "queued" && scenario.id !== "processing" && (
        <details className="mt-4 text-xs text-current/70">
          <summary className="w-fit cursor-pointer rounded-md font-medium underline-offset-4 hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-current/50">{t("conversation.technical")}</summary>
          <p className="mt-2">{t("conversation.videoIdentifier")}: {scenario.job.videoId}</p>
        </details>
      )}
    </section>
  )
