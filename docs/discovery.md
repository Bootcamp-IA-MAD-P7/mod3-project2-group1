...
| OQ-03 | CERRADA (2026-09-21) por decisión del equipo: umbrales fijados tras la evidencia DEV, ligeramente por debajo de los resultados de Logistic Regression para aceptar el candidato | Umbral mínimo de calidad y recall de odio | Aprobado: gap final `abs(train macro-F1 − test macro-F1)×100 < 5 pp` (spec ML-03); `recall hate ≥ 0.50` (DEV 0.5127); mejora sobre Dummy exigida con `test macro-F1 > 0.50` (DEV 0.6049; Dummy `most_frequent` macro-F1 DEV ≈ 0.35). Un FAIL de la corrida TEST se reporta sin re-iterar | Umbrales fijados poco antes de la corrida final; si el gate falla, promoción bloqueada y revisión humana, no tuning sobre TEST |
